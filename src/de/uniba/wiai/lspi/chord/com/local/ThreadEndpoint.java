...
public final class ThreadEndpoint extends Endpoint {
	...
	private static long EP_ID_CTR = 0;
	...
	private List<ThreadProxy> proxies = null;
	...
	public ThreadEndpoint(Node node1, URL url1) {
		...
		this.proxies = new LinkedList<ThreadProxy>();
		this.registry = Registry.getRegistryInstance();
		this.logger.info(this + " initialised.");
	}

	private static synchronized void createEndpointID(ThreadEndpoint ep) {
		ep.endpointID = ep.url.toString() + System.currentTimeMillis() + "-"
				+ EP_ID_CTR++;
	}
	...
	public Node findSuccessor(ID key) throws CommunicationException {
		...
		if (n == this.node) {
			...
			ThreadProxy t = new ThreadProxy(this.url, this.url, this
					.getNodeID(), this.endpointID);
			n = t;
		}
		this
	...
	public List<Node> notify(Node potentialPredecessor)
		...
		for (Node current : n) {
			if (current == this.node) {
				...
				this.logger
						.debug("Returned node is local node. Converting to 'remote' reference. ");
				n.add(new ThreadProxy(this.url, this.url, this.getNodeID(),
						this.endpointID));
			}
		}
	...
	public RefsAndEntries notifyAndCopyEntries(Node potentialPredecessor)
		...
		for (Node current : nodes) {
			if (current == this.node) {
				...
				this.logger
						.debug("Returned node is local node. Converting to 'remote' reference. ");
				nodes.add(new ThreadProxy(this.url, this.url, this.getNodeID(),
						this.endpointID));
			}
		}
		this
	...
	private void waitFor(int state_) throws CommunicationException {
		synchronized (this.lock) {
			while (this.getState() > STARTED && this.getState() < state_) {
				try {
					this.logger.debug(Thread.currentThread()
							+ " waiting for state: " + state_ + " on "
							+ this.getURL());
					this.lock.wait();
					if (state_ == CRASHED || state_ == STARTED) {
	...
	protected void closeConnections() {
		...
		/** state has changed. notify waiting threads */
		for (ThreadProxy p : this.proxies) {
			p.invalidate();
		}
	 ...
	 * method heavily relies on the internal structure of service layer
	...
	public void registerProxy(ThreadProxy threadProxy) {
		this.proxies.add(threadProxy);
	}
}