...
public final class RMIProxy extends Proxy {
	...
	RMIProxy(RemoteNodeInfo rNode, URL url) {
		...
		if (url == null) {
			throw new IllegalArgumentException(
					"URL of local node must not be null!");
		}
	 ...
	 */
	RMIProxy(URL localURL, URL url) throws RemoteException,
		...
		try {
			this.remoteNode = (RemoteNode) LocateRegistry.getRegistry(
					url.getHost(), url.getPort()).lookup(
					NAME_IN_REGISTRY + url.toString());
		} catch (NotBoundException e) {
			throw new CommunicationException("Cannot find stub with name "
					+ url.getHost(), e);
		}
	 ...
	 * @throws CommunicationException
	 */
	public static RMIProxy create(URL localURL, URL url)
			throws CommunicationException {
		try {
	...
	public Node findSuccessor(ID key) throws CommunicationException {
		...
		try {
			...
			throw this.handleRemoteException(e);
		}
	}
	...
	public void leavesNetwork(Node predecessor) throws CommunicationException {
		...
		try {
			...
			if (this.localURL.equals(predecessor.getNodeURL())) {
				info = new RemoteNodeInfo(this.localEndpoint.getRemoteNode(),
						predecessor.getNodeID(), predecessor.getNodeURL());
			} else {
				info = new RemoteNodeInfo(((RMIProxy) predecessor).remoteNode,
						predecessor.getNodeID(), predecessor.getNodeURL());
			}
	...
	private CommunicationException handleRemoteException(RemoteException e) {
		this.connected = false;
		this.remoteNode = null; 
		return new CommunicationException(this.localURL
				+ ": Could not connect to " + this.nodeURL + "! Message: "
				+ e.getMessage(), e);
	}
