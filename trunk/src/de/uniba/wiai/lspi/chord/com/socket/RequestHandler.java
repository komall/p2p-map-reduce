 ...
 * 
 * TODO: Waiting threads are woken up with notifyAll even if state change not
 * relevant. change use of synchronized to use of java.util.concurrent.Lock
 * and use suitable condition... 
 * 
 * @author sven
 * @version 1.0.5
 */
final class RequestHandler extends Thread implements EndpointStateListener {
	 ...
	 *             socket fails.
	 */
	RequestHandler(Node node_, Socket connection_, SocketEndpoint ep)
			throws IOException {
	...
	Serializable invokeMethod(int methodType, Serializable[] parameters)
		...
		case MethodConstants.NOTIFY: {
			RemoteNodeInfo nodeInfo = (RemoteNodeInfo) parameters[0];
			List<Node> l = this.node.notify(SocketProxy
					.create(nodeInfo.getNodeURL(), this.node.getNodeURL(),
							nodeInfo.getNodeID()));
			List<RemoteNodeInfo> nodeInfos = new LinkedList<RemoteNodeInfo>();
			for (Node current : l) {
				nodeInfos.add(new RemoteNodeInfo(current.getNodeURL(), current
						.getNodeID()));
			}
	...
	public void disconnect() {
		...
		if (this.connected) {
			...
			try {
				synchronized (this.out) {
					this.out.writeObject(new Response(
							Response.REQUEST_SUCCESSFUL,
							MethodConstants.SHUTDOWN, ""));
					this.out.close();
				}
			} catch (IOException e) {
				/* should not occur */
